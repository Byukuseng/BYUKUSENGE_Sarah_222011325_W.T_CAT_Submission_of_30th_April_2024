...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO Products(ProductID, ProductName, Price, Category) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $pid, $pname, $pprice, $pcategory);
...
include('db_connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $pid = $row['ProductID']; // Fetch the Product_Id
                echo "<tr>
                    <td>" . $row['ProductID'] . "</td>
