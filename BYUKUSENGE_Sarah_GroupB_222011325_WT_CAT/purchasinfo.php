...
<head>
  ...
  <title>Purchaseinfo table</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<h1><u> Purchaseinfo Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="purchid">PurchaseID:</label>
        <input type="number" id="purchid" name="purchid"><br><br>
        ...
        <input type="date" id="purchdate" name="purchdate" required><br><br>

        <label for="totalamnt">TotalAmount:</label>
        <input type="text" id="totalamnt" name="totalamnt" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO purchaseinfo(PurchaseID, CustomerID, EmployeeID, PurchaseDate, TotalAmount) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssd", $purchaseid, $custid, $empid, $purchasedate, $totamount);
...
// SQL query to fetch data from the purchaseinfo table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of purchaseinfo</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Purchaseinfo</h2></center>
    <table border="3">
        <tr>
            <th>PurchaseID</th>
            <th>CustomerID</th>
...
include('db_connection.php');
        ...
        // Prepare SQL query to retrieve all Purchases
        $sql = "SELECT * FROM purchaseinfo";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $purchaseid = $row['PurchaseID']; // Fetch the Purchase_Id
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_purchaseinfo.php?PurchaseID=$purchaseid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_purchaseinfo.php?PurchaseID=$purchaseid'>Update</a></td> 
                </tr>";
            }
